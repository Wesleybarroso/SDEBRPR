...
export default function Configuracoes() {
  ...
  return <DashboardLayout><div className="min-h-screen bg-[#f6f7f9] p-6"><p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-[#719080]">Centro de controle</p><h1 className="mt-2 font-serif text-4xl text-[#18352a]">Configurações de APIs</h1><p className="mt-2 max-w-2xl text-sm text-[#708078]">Gerencie cada integração separadamente. Você pode substituir ou remover uma credencial sem alterar as demais.</p><div className="mt-7 grid gap-7 xl:grid-cols-[1.5fr_0.7fr]"><Card className="border-0 shadow-sm"><CardHeader><CardTitle className="font-serif text-2xl font-normal">Integrações operacionais</CardTitle><p className="text-sm text-muted-foreground">Os valores salvos ficam criptografados no servidor e aparecem somente como máscara parcial.</p></CardHeader><CardContent><div className="space-y-3">{fields.map(([key, label, description]) => { const existing = String(configured[key] ?? ""); const isEditing = editing === key; return <div key={key} className="rounded-2xl border border-[#e1ebe4] bg-white p-4"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><p className="font-medium text-[#29483a]">{label}</p><p className="mt-1 text-xs text-[#8a9991]">{existing ? <span className="text-emerald-700">Configurado: {existing}</span> : description}</p></div><Badge className={existing ? "w-fit border-0 bg-[#e1f0e6] text-[#35604a]" : "w-fit border-0 bg-[#f1f3f2] text-[#718078]"}>{existing ? "Ativo" : "Não configurado"}</Badge></div>{(isEditing || !existing) && <div className="mt-4 flex flex-col gap-2 sm:flex-row"><Input value={form[key]} onChange={e => update(key, e.target.value)} placeholder={existing ? "Digite o novo valor para substituir" : description} type={secretField(key) ? "password" : "text"} className="border-[#dce7df] bg-[#fbfcfb]" /><Button onClick={() => saveField(key)} disabled={save.isPending} className="bg-[#18352a] hover:bg-[#285642]">{save.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Salvar</Button></div>}{existing && <div className="mt-4 flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={() => setEditing(isEditing ? null : key)} className="border-[#cddbd1] text-[#355b47]"><Pencil className="mr-2 h-3.5 w-3.5" />{isEditing ? "Cancelar edição" : "Editar / substituir"}</Button><Button variant="outline" size="sm" onClick={() => removeField(key, label)} disabled={remove.isPending} className="border-red-200 text-red-700 hover:bg-red-50"><Trash2 className="mr-2 h-3.5 w-3.5" />Remover</Button></div>}</div>; })}</div></CardContent></Card><Card className="h-fit border-0 bg-[#18352a] text-white shadow-sm"><CardContent className="p-6"><ShieldCheck className="h-7 w-7 text-[#b7dbc4]" /><h2 className="mt-5 font-serif text-2xl">Segurança por padrão.</h2><p className="mt-3 text-sm leading-6 text-[#b8cabe]">Remover uma credencial limpa somente aquele campo. As outras integrações continuam intactas, e nenhum segredo completo é enviado para o navegador.</p><div className="mt-6 flex items-center gap-2 text-xs text-[#9ec0a9]"><Badge className="border-0 bg-white/10 text-[#c9e5d1]">{query.data?.configured ? "Configuração disponível" : "Aguardando configuração"}</Badge></div></CardContent></Card></div></div></DashboardLayout>;
}
